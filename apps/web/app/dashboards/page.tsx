  ...
  const filtered = useMemo(
    () =>
      (dashboards.data ?? []).filter((dashboard) => {
        ...
        const text =
          `${dashboard.site_name ?? ''} ${dashboard.site_reference ?? ''} ${dashboard.device_name ?? ''} ${dashboard.device_code ?? ''} ${dashboard.name}`.toLowerCase();
    ...
    <>
      <div className="heading">
        <div>
          ...
          <h1>Painéis</h1>
          <p>Um painel é criado automaticamente para cada equipamento.</p>
        </div>
      </div>
      <div className="dashboard-filters">
        <input
          ...
          placeholder="Buscar cliente, equipamento ou código…"
        />
      ...
      <div className="dashboard-list">
        {filtered.map((dashboard) => (
          <Link
            ...
            className={`dashboard-list-card ${dashboard.device_deactivated ? 'deactivated' : ''}`}
            key={dashboard.id}
          >
            ...
            <div className="dashboard-card-copy">
              <span className="widget-kicker">
                {dashboard.site_name ?? 'CLIENTE'} · {dashboard.site_reference ?? 'SEM REFERÊNCIA'}
              </span>
              <h2>{dashboard.name}</h2>
              <p>
                <b>{dashboard.device_name ?? 'Múltiplos ativos'}</b>{' '}
                <span className="code-chip">{dashboard.device_code}</span>
              </p>
              <small>
                Último sinal: {time(dashboard.last_message_at)} · {dashboard.widget_count ?? 0}{' '}
                indicadores
              </small>
            </div>
            <span
              className={`dashboard-status ${dashboard.device_deactivated ? 'deactivated' : dashboard.device_online ? 'online' : 'offline'}`}
            >
              {dashboard.device_deactivated
                ? 'Desativado'
                : dashboard.device_online
                  ? 'Online'
                  : 'Offline'}
            </span>
            <b>→</b>
        ...
        {dashboards.data && !filtered.length && (
          <div className="empty">Nenhum painel corresponde aos filtros selecionados.</div>
        )}
      </div>
    </>
  );
