...
const allowed =
  /^(?:health|overview|tenants|sites(?:\/[0-9a-f-]+\/shifts)?|devices(?:\/[0-9a-f-]+(?:\/(?:tags(?:\/[0-9a-f-]+\/(?:history|hmi-check|decimals))?|latest|signals|statistics|mqtt-credential|production-context|production-settings|production-overview|hidden-products|legacy-attempts|legacy-allow|production-config|production-detail|production-target|shift-board|shift-detail|shift-reports(?:\/(?:snapshot|recalculate))?))?)?|telemetry|mqtt\/(?:raw|topics)|dashboards(?:\/[0-9a-f-]+(?:\/(?:layout|statistics|counters|template(?:\/apply)?|tv(?:\/apply-template)?|snapshots(?:\/[0-9a-f-]+\/restore)?|widgets(?:\/[0-9a-f-]+(?:\/reset-counter)?)?))?)?|export\/telemetry\.csv|users(?:\/[0-9a-f-]+(?:\/reset-password)?)?|branding(?:\/public)?)$/;
