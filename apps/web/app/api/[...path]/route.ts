...
const allowed =
  /^(?:health|overview|operations\/(?:overview|settings)|tenants|sites(?:\/[0-9a-f-]+\/(?:shifts|location))?|devices(?:\/[0-9a-f-]+(?:\/(?:location|operation-card|raw-capture|tags(?:\/[0-9a-f-]+\/(?:history|hmi-check|decimals))?|latest|signals|statistics|mqtt-credential|production-context|production-settings|production-overview|hidden-products|legacy-attempts|legacy-allow|production-config|production-detail|production-photo|production-target|shift-board|shift-detail|shift-minutes|shift-reports(?:\/(?:snapshot|recalculate))?))?)?|telemetry|mqtt\/(?:raw|topics)|dashboards(?:\/[0-9a-f-]+(?:\/(?:layout|statistics|counters|template(?:\/apply)?|tv(?:\/apply-template)?|snapshots(?:\/[0-9a-f-]+\/restore)?|widgets(?:\/[0-9a-f-]+(?:\/reset-counter)?)?))?)?|export\/telemetry\.csv|users(?:\/[0-9a-f-]+(?:\/reset-password)?)?|branding(?:\/public)?)$/;
